...
const Signup = () => {
    ...
    const 아이디중복검사 = (eventId) => {
        ...
        .then(result => {
            ...
            // Number(result) === 0 나중에 Spring Boot에서 아이디가 중복되지 않으면 0을 전달
            // 아이디가 중복이면 0 이외 값을 전달
            // 결과전달을 0을 사용하지 않고 "중복아님"글자 전달 
            if(Number(result) === 0) {
    ...
    const 회원가입버튼 = () => {
        ...
        */
        ...
        fetch("/signUp", {
            method : "POST", //Spring Boot Container에 @PostMapping("/signUp") 에 전달하겠다는 표시
            headers : {"Content-Type" : "application/json"},
            //<form> 태그에는 "Content-Type" : "application/json" 기본으로 생략돼서 작성
            // Content-Type은 왜 작성해야 하는가?
            // 데이터를 전달할 때 이미지파일, 동영상 파일인지, 다중이미지파일인지 글자만 있는 정보 전달
            // application/json : application = 코딩하는폴더 자체를 의미 폴더1개 = 어플리케이션 1개
            //                           json = 사용자나 개발자가 작성한 여러값을 key-value형태로 데이터 1개이상 주고받을 수 있는 형태 
